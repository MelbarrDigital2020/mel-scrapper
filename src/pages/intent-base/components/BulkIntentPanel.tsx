...
import type { BulkIntentJob, JobStatus } from "./intentbase.types";
import { cn, jobMeta } from "./intentbase.utils";
import { DonutCard, Pager } from "./intentbase.ui";

export default function BulkIntentPanel() {
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const [bulkFile, setBulkFile] = useState<File | null>(null);
  const [bulkLoading, setBulkLoading] = useState(false);

  const [jobs, setJobs] = useState<BulkIntentJob[]>(() => {
    const base: BulkIntentJob[] = Array.from({ length: 22 }).map((_, i) => {
      const total = 500 + Math.floor(Math.random() * 2500);
      const high = Math.floor(total * (0.15 + Math.random() * 0.2));
      const medium = Math.floor(total * (0.2 + Math.random() * 0.25));
      const low = Math.floor(total * (0.15 + Math.random() * 0.25));
      const unknown = Math.max(0, total - high - medium - low);
  ...
  const bulkCounts = useMemo(() => {
    const latest = jobs[0];
    if (!latest) return { high: 0, medium: 0, low: 0, unknown: 0 };
    return {
      high: latest.high,
  ...
  const downloadJob = (job: BulkIntentJob) => {
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div className="lg:col-span-7">
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-sm font-semibold">Intent Bulk</h2>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                Upload a CSV file, run intent scoring, then download results.
              </p>
            </div>
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-1 text-xs text-gray-600 dark:text-gray-300">
                      Click to select. Make sure an <b>email</b> column exists.
                    </p>
            ...
            <div className="sm:col-span-1">
              <button
                ...
                type="button"
              >
                {bulkLoading ? "Uploading..." : "Run Intent Bulk"}
              </button>
          ...
          <div className="mt-6 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-950">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Recent jobs overview (high vs medium vs low vs unknown).
                </p>
              </div>
            ...
            <div className="mt-4 h-48 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trendData}>
                  ...
                  <Line type="monotone" dataKey="medium" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="low" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="unknown" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      <div className="lg:col-span-5">
        <DonutCard
          ...
          high={bulkCounts.high}
          medium={bulkCounts.medium}
          low={bulkCounts.low}
          unknown={bulkCounts.unknown}
        />
      </div>
      ...
      <div className="lg:col-span-12">
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h3 className="text-sm font-semibold">Intent Bulk History</h3>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  {pagedJobs.map((j) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right">{j.high}</td>
                        <td className="px-4 py-3 text-right">{j.medium}</td>
                        <td className="px-4 py-3 text-right">{j.low}</td>
                        <td className="px-4 py-3 text-right">{j.unknown}</td>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-4">
            ...
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-950/40">
              ...
              <p className="mt-1 text-xs text-gray-600 dark:text-gray-300">
                Consider nurture or deprioritize.
              </p>
            </div>
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-950/40">
              <div className="flex items-center gap-2 text-sm font-semibold">
